...
async function importar(req, res) {
  ...
  for (let i = 0; i < lista.length; i++) {
    ...
    try {
      // Curso com escopo de instituição para garantir multi-tenancy
      let curso = await prisma.curso.findFirst({
        where: {
          codigo: item.curso.codigo,
          alunos: { some: { instituicaoId: req.institutionId } },
        },
      });
      ...
      if (!curso) {
        curso = await prisma.curso.create({
          data: {
            nome: item.curso.nome, codigo: item.curso.codigo,
            dtInicio: new Date(item.curso.dt_inicio), dtFim: new Date(item.curso.dt_fim),
  ...
  return res.status(201).json({
    status: "success", importados: resultados.length,
    erros: errosGerais, data: resultados,
  });
}
